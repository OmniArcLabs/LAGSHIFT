...
<p>تشخیص خودکار اپراتور فقط با اجازه جداگانه یک درخواست کوچک به Cloudflare می‌فرستد؛ سرویس مقصد IP اتصال را طبق ماهیت اینترنت می‌بیند، اما LAGSHIFT آن را نگه نمی‌دارد. استعلام کمکی LinkIrani نیز جداگانه فعال می‌شود و فقط نام میزبان نهایی را بدون مسیر فایل، query یا token می‌فرستد؛ پاسخ آن غیرتضمینی است.</p>
<p>مقایسهٔ IP میزبان با فهرست آفلاین تخصیص‌های ایرانِ RIPE NCC روی دستگاه انجام می‌شود و درخواستی برای RIPE ارسال نمی‌کند. کشور تخصیص IP فقط نشانهٔ فنی است و تعرفهٔ اپراتور را اثبات نمی‌کند.</p>
