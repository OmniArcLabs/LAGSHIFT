...
## امکانات نسخه عمومی 1.3

- FusionDNS Hybrid: مقایسهٔ DNSهای ایرانی و خارجی، سنجش پوشش تک‌تک
  مقصدهای برنامه و انتخاب یک مسیر کامل و تأییدشده
...
- تشخیص محافظه‌کار «تعرفهٔ داخلی» برای لینک سایت یا فایل: دنبال‌کردن Redirect،
  بررسی میزبان نهایی و IPv4/IPv6، فهرست آفلاین تخصیص‌های ایران از RIPE NCC،
  تشخیص اختیاری اپراتور، استعلام کمکی و فقط-میزبان LinkIrani، هشدار WARP/VPN
  و رسید پاک‌سازی‌شده برای پیگیری ۱۹۵
...
در ابزار تعرفه، query و token لینک فقط برای همان درخواست در حافظه استفاده می‌شود و
در نتیجه، تاریخچه یا رسید ذخیره نمی‌شود. هیچ لینک کاربر نیز بدون رضایت به سرویس
استعلام ثالث فرستاده نمی‌شود. فهرست آفلاین RIPE NCC بدون هیچ درخواست خارجی فقط
کشور تخصیص ثبتی IP را بررسی می‌کند؛ این موقعیت، همان تأیید تعرفه نیست. ثبت دامنه
یا IP در فهرست تعرفهٔ داخلی نیز ضریب نهایی کسر بسته توسط اپراتور را تضمین نمی‌کند.
...
می‌شود و دانلود قطع‌شده قابل ادامه است. فایل‌های 1.1.0 هنوز گواهی پولی
